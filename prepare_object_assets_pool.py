...
def resolve_pool_filter_cfg(cfg: dict) -> dict:
    ...
    version = str(pool_cfg.get("version", "v1")).strip()
    ...
    if not version:
        raise ValueError("Config field pool_filter.version must be non-empty.")
    if mesh_kind not in {"coacd", "manifold"}:
        raise ValueError(
            "Config field pool_filter.mesh_kind must be one of: coacd, manifold."
        )
    for field_name, value in [
        ("pool_filter.thin_ratio_min", thin_ratio_min),
        ("pool_filter.flat_ratio_min", flat_ratio_min),
    ]:
...
def _load_mesh_as_trimesh(mesh_path: Path) -> trimesh.Trimesh:
...
def _pool_filter_metrics(source_dir: Path, mesh_kind: str) -> dict:
    if mesh_kind == "coacd":
        mesh_path = ScaleDatasetBuilder.coacd_path(source_dir)
    else:
        mesh_path = ScaleDatasetBuilder.manifold_path(source_dir)
    if not mesh_path.exists():
        raise FileNotFoundError(f"Pool filter mesh not found: {mesh_path}")

    mesh = _load_mesh_as_trimesh(mesh_path)
    extents = np.sort(np.asarray(mesh.extents, dtype=np.float64))
    if extents.shape != (3,) or float(extents[2]) <= 1e-12:
        raise ValueError(f"Invalid mesh extents for pool filter: {mesh_path}")

    return {
        "mesh_path": str(mesh_path.resolve()),
        "thin_ratio": float(extents[0] / extents[2]),
        "flat_ratio": float(extents[1] / extents[2]),
        "extents_sorted": [float(extents[0]), float(extents[1]), float(extents[2])],
    }


def _pool_filter_keep(metrics: dict, pool_filter_cfg: dict) -> bool:
    return bool(
        float(metrics["thin_ratio"]) >= float(pool_filter_cfg["thin_ratio_min"])
        and float(metrics["flat_ratio"]) >= float(pool_filter_cfg["flat_ratio_min"])
    )
...
def _load_rebuild_tasks(cfg: dict, config_path: str) -> list[dict]:
    ...
    for obj in obj_iter:
        ...
        if bool(pool_filter_cfg["enabled"]):
            metrics = _pool_filter_metrics(
                object_dir, mesh_kind=str(pool_filter_cfg["mesh_kind"])
            )
            if not _pool_filter_keep(metrics, pool_filter_cfg):
                filtered_count += 1
                continue
    ...
    if bool(pool_filter_cfg["enabled"]):
        print(
            f"[prepare_object_assets_pool] pool_filter version={pool_filter_cfg['version']} "
            f"mesh_kind={pool_filter_cfg['mesh_kind']} thin_ratio_min={pool_filter_cfg['thin_ratio_min']:.3f} "
            f"flat_ratio_min={pool_filter_cfg['flat_ratio_min']:.3f} "
            f"kept={len(tasks)} filtered={filtered_count}"
        )
    return tasks
...
def main() -> None:
    ...
    if args.force:
        ...
        print(
            f"[prepare_object_assets_pool] rebuild objects={len(object_tasks)} "
        ...
        print(
            f"[prepare_object_assets_pool] validate entries={len(entries)} objdata_tag={objdata_tag}"
        )
    ...
    print(
        f"[prepare_object_assets_pool] objdata_tag={objdata_tag} entries={len(entries)} "
