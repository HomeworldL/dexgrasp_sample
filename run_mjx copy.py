...
def run_sampling_mjx(
    ...
    mjx_impl = str(mjx_cfg.get("impl", "warp"))
    ...
    # Fail-fast policy: any MJX init/runtime issue should terminate this run.
    runner = MjHOMJX(
        obj_info,
        hand_xml_path,
    ...
    with h5py.File(h5_path, "w") as hf:
        ...
        for start in pbar:
            ...
            if mask_precheck.any():
                ...
                print(f"Precheck passed: {mask_precheck.sum()} / {len(mask_precheck)} in batch {start}-{end}")
    #             passed = {
    ...
    #             q_grasp_tail, contact_tail = runner.sim_grasp_batch(
    #                 sim_pool["qpos_prepared"], sim_grasp_cfg=sim_grasp_cfg
    #             )
