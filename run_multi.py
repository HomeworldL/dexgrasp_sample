...
def run_one(obj_name: str, script: str, python_exe: str = sys.executable, logs_dir: Path = Path("logs")):
    """
    启动子进程执行: python_exe script -o obj_name
    把 stdout/stderr 写入 logs/<obj_name>.log
    返回 (obj_name, returncode, logpath)
    """
    cmd = [python_exe, script, "-o", obj_name]
...
def main():
    ...
    names = get_all_object_names()
    if not names:
        print("没有发现任何对象（ds.id2name 为空），退出。")
        return
    exclude_names = {"002_master_chef_can", "004_sugar_box", "013_apple", "035_power_drill"}
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.max_parallel) as ex:
            for name in names:
                futures.append(ex.submit(run_one, name, args.script))
