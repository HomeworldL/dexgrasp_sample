...
CONFIG_DIR = Path("scripts/configs/solref_only_cases").resolve()
WORK_DIR = Path("tmp/solref_only_sweep_YCB_013_apple_scale080").resolve()
OUT_DIR = WORK_DIR / "outputs"
PC_SUBDIR = "pc_warp"
LOG_DIR = WORK_DIR / "logs"

SOLIMP_BASE = [0.9, 0.95, 0.001, 0.5, 2.0]
...
def build_cfg(base_cfg: dict, timeconst: float) -> dict:
    ...
    cfg["hand"]["profile"]["solimp"] = SOLIMP_BASE
    cfg["profile_object"]["solimp"] = SOLIMP_BASE

    solref = [float(timeconst), float(DAMP_RATIO)]
    # hand/object use identical values for controlled comparison
    cfg["hand"]["profile"]["solref"] = solref
    cfg["profile_object"]["solref"] = solref
    return cfg
...
def run_case(case_name: str, cfg_path: Path) -> dict:
    ...
    rec = {
        "case": case_name,
        "timeconst": float(case_name.split("_")[-1]),
...
def main() -> None:
    ...
    for tc in SOLREF_TIMECONST_LIST:
        name = f"solref_{tc}"
        cfg = build_cfg(base_cfg, tc)
    ...
    if args.skip_run:
        for name, path in cfg_paths.items():
            ...
            rec = {
                "case": name,
                "timeconst": float(name.split("_")[-1]),
                "return_code": 0,
                "wall_time_sec": None,
    ...
    records.sort(key=lambda x: x["timeconst"])
