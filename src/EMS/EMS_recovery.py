...
# from superquadrics import rotations, superquadric
from src.EMS.superquadrics import rotations
...
def EMS_recovery(
    ...
    # print("DEBUG: np.asarray -> dtype:", point.dtype, "ndim:", point.ndim, "shape:", point.shape)
    # print(" - flags:", point.flags)
    # # 强制转换为 C-contiguous float64
    # point = np.ascontiguousarray(point, dtype=np.float64)
    # print("DEBUG AFTER CAST: dtype:", point.dtype, "C_CONTIGUOUS:", point.flags['C_CONTIGUOUS'])
    t0 = np.mean(point, 0)
    point = point - t0
