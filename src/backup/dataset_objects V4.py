...
from src.EMS import EMS_recovery
...
class DatasetObjects:
    ...
    def get_mesh(self, name: str, mesh_type: str = "inertia", alpha: float = 1.0) -> trimesh.Trimesh:
        if mesh_type not in ("inertia", "simplified", "manifold", "coacd", "convex_pieces"):
        ...
        if mesh_type == "convex_pieces":
            ...
            for piece in pieces:
                ...
                rgba = np.tile(np.concatenate([rgb, np.array([alpha_byte], dtype=np.uint8)]), (n_verts, 1))
        ...
        if 0.0 <= alpha < 1.0:
            ...
            if getattr(mesh.visual, "vertex_colors", None) is not None and mesh.visual.vertex_colors.size:
                ...
                if vc.shape[1] == 3:
                    vc = np.concatenate([vc, np.full((vc.shape[0],1), 255, dtype=np.uint8)], axis=1)
                vc[:, 3] = a_byte
                mesh.visual.vertex_colors = vc
            else:
                mesh.visual.vertex_colors = np.tile(np.array([255,255,255,a_byte], dtype=np.uint8), (n,1))
        return mesh
    ...
    def sample_surface_o3d(
        self, obj_path: str, n_points: int = 4096, method: str = "uniform"
    ...
    def compute_superdec_datasets(
        ...
        for i0 in range(0, len(all_names), BATCH_SIZE):
            ...
            for name in chunk_names:
                ...
                input_mesh = info.get("inertia_abs") or info.get("simplified_abs") or info.get("manifold_abs")
                if not input_mesh or not os.path.exists(input_mesh):
            ...
            batch_np = np.stack([p if p.shape[0] == N else np.pad(p, ((0, N - p.shape[0]), (0, 0))) for p in pts_norm_list], axis=0)  # (B,N,3)
            ...
            points_den = denormalize_points(points_batch_t.cpu(), translation_arr, scale_arr, z_up=False)  # torch tensor on CPU
            ...
            # for each object in this chunk, create sd_result and save to npz
            for idx_in_chunk, name in enumerate(valid_names):
                info = info_list[idx_in_chunk]
                try:
                    pcd = pts_den_np[idx_in_chunk]  # (N,3)
                    assign_matrix = assign_matrix_all[idx_in_chunk] if assign_matrix_all is not None else None  # (N,P)
                    exist = exist_all[idx_in_chunk] if exist_all is not None else np.zeros(P)
                    exist_mask = (exist > 0.5) # TODO: check this
                    exist_indices = np.nonzero(exist_mask)[0].tolist()

                    if len(exist_indices) == 0:
                        prim_scale = np.zeros((0, 3), dtype=float)
                        prim_rotate = np.zeros((0, 3, 3), dtype=float)
                        prim_trans = np.zeros((0, 3), dtype=float)
                        prim_shape = np.zeros((0, 2), dtype=float)
                        assign_trim = assign_matrix  # keep full assign_matrix
                        segmentation = np.zeros(pcd.shape[0], dtype=np.int32)
                    else:
                        prim_scale = scale_all[idx_in_chunk][exist_indices] if scale_all is not None else None
                        prim_rotate = rotate_all[idx_in_chunk][exist_indices] if rotate_all is not None else None
                        prim_trans = trans_all[idx_in_chunk][exist_indices] if trans_all is not None else None
                        prim_shape = shape_all[idx_in_chunk][exist_indices] if shape_all is not None else None
                        # trimmed assign_matrix columns correspond to existing primitives
                        assign_trim = assign_matrix[:, exist_indices] if assign_matrix is not None else None
                        # segmentation by argmax on trimmed assign matrix
                        if assign_trim is not None:
                            seg_labels = np.argmax(assign_trim, axis=1).astype(np.int32)
                            segmentation = seg_labels
                        else:
                            segmentation = np.zeros(pcd.shape[0], dtype=np.int32)

                    # prepare npz to save
                    # determine save folder: same dir as inertia_abs if available, else folder_abs
                    save_dir = None
                    if info.get("inertia_abs") and os.path.exists(info.get("inertia_abs")):
                        save_dir = os.path.dirname(info.get("inertia_abs"))
                    else:
                        save_dir = info.get("folder_abs", os.path.dirname(info.get("inertia_abs") or ""))

                    if not save_dir:
                        # fallback to dataset root/name
                        save_dir = info.get("folder_abs", self.root)

                    os.makedirs(save_dir, exist_ok=True)
                    npz_path = os.path.join(save_dir, "superdec.npz")

                    # Arrays to store (use numpy arrays)
                    save_dict = {
                        "pcd": np.asarray(pcd),
                        "assign_matrix": np.asarray(assign_trim) if assign_trim is not None else np.asarray([]),
                        "exist_indices": np.asarray(exist_indices, dtype=np.int32),
                        "primitive_count_total": np.asarray(P, dtype=np.int32),
                        "primitive_count_exist": np.asarray(len(exist_indices), dtype=np.int32),
                        "scale": np.asarray(prim_scale),
                        "rotation": np.asarray(prim_rotate),
                        "translation": np.asarray(prim_trans),
                        "exponents": np.asarray(prim_shape),
                        "exist": np.asarray(exist),
                        "segmentation": np.asarray(segmentation, dtype=np.int32),
                    }

                    # save compressed npz (overwrite if exists)
    ...
    def load_superdec_npz(self, object_name: str) -> Dict[str, Any]:
        """
        Load an existing superdec.npz for object `object_name` and return a dictionary with standardized keys.

        The function is tolerant to different saved key names:
        - expected keys in our pipeline: 'pcd','assign_matrix','exist_indices','scale','rotation','translation','exponents','exist','segmentation'
        - older/alternate formats (from other code) might use: 'pc','assign_matrix','scale','rotation','translation','exponents','exist','names'
        Returns:
        A dict with keys:
            'name' (str),
            'pcd' (numpy array Nx3),
            'assign_matrix' (numpy array NxP) or None,
            'exist' (numpy array P) or None,
            'exist_indices' (list) or derived from 'exist' if available,
            'scale','rotation','translation','exponents' (numpy arrays) or None,
            'segmentation' (numpy array N) or None,
            'npz_path' absolute path
        Raises:
        FileNotFoundError if file missing, ValueError for unreadable content.
        """
        if object_name not in self.index:
        ...
        if not npz_path:
            # fallback to inertia_abs sibling folder
            folder = info.get("folder_abs")
            inertia = info.get("inertia_abs")
            if inertia and os.path.exists(inertia):
                folder = os.path.dirname(inertia)
            if not folder:
                raise FileNotFoundError(f"No superdec path or folder found for {object_name}")
            npz_path = os.path.join(folder, "superdec.npz")
        npz_path = os.path.abspath(npz_path)
        if not os.path.exists(npz_path):
            raise FileNotFoundError(f"superdec npz not found: {npz_path}")
        ...
        out = {"name": object_name, "npz_path": npz_path}

        # pcd
        k = get_key("pcd", "pc")
        out["pcd"] = data[k] if k is not None else None

        # assign_matrix
        k = get_key("assign_matrix", "assign")
        out["assign_matrix"] = data[k] if k is not None else None

        # segmentation (explicit)
        k = get_key("segmentation")
        out["segmentation"] = data[k] if k is not None else None

        # exist array or exist_indices
        k_exist = get_key("exist")
        k_exist_indices = get_key("exist_indices")
        if k_exist is not None:
            out["exist"] = data[k_exist]
            out["exist_indices"] = np.nonzero(out["exist"] > 0.5)[0].tolist()
        elif k_exist_indices is not None:
            out["exist_indices"] = data[k_exist_indices].tolist()
            # reconstruct exist array from count if primitive_count_total exists
            k_total = get_key("primitive_count_total", "P", "primitive_total")
            if k_total is not None:
                P = int(np.asarray(data[k_total]).item())
                exist = np.zeros((P,), dtype=np.float32)
                exist[out["exist_indices"]] = 1.0
                out["exist"] = exist
            else:
                out["exist"] = None
        else:
        ...
        # params
        out["scale"] = data[get_key("scale")] if get_key("scale") else None
        out["rotation"] = data[get_key("rotation")] if get_key("rotation") else None
        out["translation"] = data[get_key("translation")] if get_key("translation") else None
        out["exponents"] = data[get_key("exponents", "shape")] if get_key("exponents", "shape") else None

        # if assign_matrix exists and segmentation not present, compute segmentation
        if out["segmentation"] is None and out["assign_matrix"] is not None:
            try:
                assign = np.asarray(out["assign_matrix"])
                if assign.ndim == 2:
                    seg = np.argmax(assign, axis=1).astype(np.int32)
                    out["segmentation"] = seg
                elif assign.ndim == 3:
                    # if batch style, pick first
                    seg = np.argmax(assign[0], axis=1).astype(np.int32)
                    out["segmentation"] = seg
            except Exception:
