...
class MjHO:
    def __init__(
        ...
        friction_coef: Sequence[float] = (1.0, 1.0),
        ...
        if target_body_params is not None:
            ...
            self.target_body_weights = np.array(list(target_body_params.values()))
            self.tip_body_ids = []
    ...
    def sim_grasp(
        ...
        # if Mp <= 0 or Mn <= 0:
        #     raise ValueError("Mp and Mn must be positive integers")
        # if Mp > N:
        #     raise ValueError("Mp must be <= number of object points")
        # if Mn > N:
        #     Mn = N
            
        # assume user already called set_hand_qpos(...) before calling this (per your note)
        # prepare history
        history = {"qpos": [], "nearest_pts": [], "tip_positions": []}

        # Main loop
        for step_i in range(int(steps)):

            hand_qpos = self.get_hand_qpos().copy()  # 当前 hand qpos（长度 self.nq_hand）

            # compute fingertip world positions (use data.xpos[body_id])
            tip_positions = []
            for bid in self.tip_body_ids:
                tip_positions.append(np.array(self.data.xpos[bid]).copy())
            tip_positions = np.vstack(tip_positions)  # (n_tips, 3)

            # find for each tip the nearest point in obj_pts
            # fast vectorized
            # compute pairwise squared distances
            # result: for each tip, index of nearest obj_pt
            dif = obj_pts[None, :, :] - tip_positions[:, None, :]  # (n_tips, N, 3)
            d2 = np.sum(dif * dif, axis=2)  # (n_tips, N)
            idx = np.argmin(d2, axis=1)  # (n_tips,)
            nearest_pts = obj_pts[idx]  # (n_tips, 3)

            history["tip_positions"].append(tip_positions.copy())
            history["nearest_pts"].append(nearest_pts.copy())
            history["qpos"].append(hand_qpos.copy())

            # print(f"step_i: {step_i}")
            # print(f"nearest_pts: {nearest_pts}")
            # print(f"tip_positions: {tip_positions}")
            # print(f"hand_qpos: {hand_qpos}")

            # return tip_positions.copy(), nearest_pts.copy()

            # construct desired tip velocities (world frame)
            v_tips = speed_gain * (nearest_pts - tip_positions)  # (n_tips, 3)
            # clip per-tip speed
