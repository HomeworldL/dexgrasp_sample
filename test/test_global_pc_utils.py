...
def test_write_global_pc_writes_float32_points(tmp_path: Path):
    ...
    normals_path = write_global_normals(np.ones_like(points), str(output_dir), "pc_warp")

    assert path == global_pc_path(str(output_dir), "pc_warp")
    assert normals_path == global_normals_path(str(output_dir), "pc_warp")
    ...
    loaded_points, loaded_normals = load_global_pc_and_normals(str(output_dir), "pc_warp")
    assert np.allclose(loaded_points, points.astype(np.float32))
    assert np.allclose(loaded_normals, np.ones_like(points, dtype=np.float32))
