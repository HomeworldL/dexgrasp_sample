...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize clustered object thumbnails by cluster."
    )
...
def _resolve_cluster_mesh_path(
    objdata_root: Path, object_name: str, scale_tag: str
) -> tuple[Path, float]:
    object_root = objdata_root / object_name
    scale_value = parse_scale_tag(scale_tag)
    if scale_value is None:
        return object_root / "meshes" / "coacd.obj", 1.0
    return object_root / "meshes_normalized" / "coacd.obj", float(scale_value)


def _load_mesh_points(mesh_path: Path, scale: float = 1.0) -> np.ndarray:
    ...
    if not np.isfinite(scale_value) or scale_value <= 0.0:
        raise ValueError(f"Invalid mesh scale for {mesh_path}: {scale}")
...
def _draw_top_view(
    ax: plt.Axes, points: np.ndarray, point_size: float, highlight: bool
) -> None:
    xy = points[:, :2]
...
def _render_cluster_pages(
    ...
    center_object_name = (
        ...
        if len(members) == 0
            ...
            min(members, key=lambda item: int(item["rank_in_cluster"]))["object_name"]
        )
    )
    ...
    for page_idx in range(total_pages):
        ...
        for flat_idx, ax in enumerate(axes_arr.flat):
            ...
            mesh_path, mesh_scale = _resolve_cluster_mesh_path(
                objdata_root=objdata_root,
                object_name=object_name,
                scale_tag=scale_tag,
            )
            points = _load_mesh_points(mesh_path, scale=mesh_scale)
            is_center = (
                center_object_name is not None and object_name == center_object_name
            )
            _draw_top_view(
                ax, points=points, point_size=point_size, highlight=is_center
            )
            title = (
        ...
        out_path = (
            output_dir / f"cluster_{int(cluster_id):02d}_page_{page_idx + 1:02d}.png"
        )
...
def main() -> None:
    ...
    scale_tag = str(
        cluster_payload.get("scale_tag", resolve_shape_cluster_cfg(cfg)["scale_tag"])
    )
    ...
    if args.cluster_id is not None:
        ...
        if target_cluster_ids[0] not in clusters:
            raise KeyError(
                f"Cluster id {args.cluster_id} not found in {cluster_labels_path}"
            )
